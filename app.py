...
model = None
if os.path.exists("model_gats_gossos.json") and os.path.exists("model_gats_gossos.weights.h5"):
    with open("model_gats_gossos.json", "r") as json_file:
        model_json = json_file.read()
    model = model_from_json(model_json)
    model.load_weights("model_gats_gossos.weights.h5")
else:
    st.warning("⚠️ El model no està disponible. Només es mostrarà la imatge, però no es podrà fer la predicció.")
...
if uploaded_file:
    try:
        ...
        if model:
            ...
            st.info("🔍 Model no carregat, no es pot fer la predicció.")
